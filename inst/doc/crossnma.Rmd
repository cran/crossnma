...
vignette: >
  ...
  %\VignetteIndexEntry{Cross-Design & Cross-Format Network Meta-Analysis and Regression}
...
```{r mm, message = FALSE, include=FALSE}
library(crossnma)
```
...
The data we use are fictitious but have been developed to resample to real RCTs with IPD and aggregate data included in @Tramacere15. The studies provide either aggregate data `stddata` (2 RCTs) or as individual participant data `ipddata` (3 RCTs and 1 cohort study). Both datasets compare in total four drugs which are anonymized.
